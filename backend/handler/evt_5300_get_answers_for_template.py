...
class Handler(EventHandler):
    ...
    async def handle(self, event, output):
        output.set("Answers", await self.get_answers_for_template(**event.data))

    async def get_answers_for_template(self, ProjectName, TemplateName):
        aids = await self.r_ans.get_ids_for_pn_tn(ProjectName, TemplateName)
